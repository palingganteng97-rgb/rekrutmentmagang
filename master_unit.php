...
// 2. FITUR PENCARIAN DATA UNIT
$keyword = "";
if (isset($_POST['cari'])) {
    $keyword = mysqli_real_escape_string($koneksi, $_POST['keyword']);
    $query = "SELECT id, kode_unit, nama_unit, status, created_at FROM mst_unit 
              WHERE kode_unit LIKE '%$keyword%' OR nama_unit LIKE '%$keyword%' 
              ORDER BY id DESC";
} else {
    $query = "SELECT id, kode_unit, nama_unit, status, created_at FROM mst_unit ORDER BY id DESC";
}
$hasil = mysqli_query($koneksi, $query);

// 3. FITUR HAPUS DATA UNIT
if (isset($_GET['hapus'])) {
    $id_hapus = mysqli_real_escape_string($koneksi, $_GET['hapus']);
    $query_hapus = "DELETE FROM mst_unit WHERE id = '$id_hapus'";
    if (mysqli_query($koneksi, $query_hapus)) {
        echo "<script>alert('Data unit berhasil dihapus!'); window.location='master_unit.php';</script>";
    }
}

// 4. FITUR UPDATE / EDIT DATA UNIT (MODAL SIMPAN)
if (isset($_POST['update_unit'])) {
    $id_edit   = mysqli_real_escape_string($koneksi, $_POST['id_unit']);
    $kode_edit = mysqli_real_escape_string($koneksi, $_POST['kode_unit']);
    $nama_edit = mysqli_real_escape_string($koneksi, $_POST['nama_unit']);
    $stat_edit = mysqli_real_escape_string($koneksi, $_POST['status_unit']);
    
    $query_update = "UPDATE mst_unit SET kode_unit='$kode_edit', nama_unit='$nama_edit', status='$stat_edit', updated_at=NOW() WHERE id='$id_edit'";
    if (mysqli_query($koneksi, $query_update)) {
        echo "<script>alert('Data unit berhasil diperbarui!'); window.location='master_unit.php';</script>";
    }
}

// 5. FITUR TAMBAH DATA UNIT BARU (MODAL TAMBAH)
if (isset($_POST['simpan_unit_baru'])) {
    $kode_baru  = mysqli_real_escape_string($koneksi, $_POST['kode_baru']);
    $nama_baru  = mysqli_real_escape_string($koneksi, $_POST['nama_baru']);
    
    $query_tambah = "INSERT INTO mst_unit (kode_unit, nama_unit, status, created_at) VALUES ('$kode_baru', '$nama_baru', 'Aktif', NOW())";
    if (mysqli_query($koneksi, $query_tambah)) {
        echo "<script>alert('Unit baru berhasil ditambahkan!'); window.location='master_unit.php';</script>";
    }
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Master Unit - Magang ID</title>
    <style>
...
<body>
    ...
    <div class="dashboard-container">
        ...
        <aside class="sidebar-left">
            <div>
                ...
                <nav class="menu-list">
                    ...
                    <a href="master_unit.php" class="menu-item active">Master Unit</a>
        ...
        <main class="main-content">
            <div class="content-header">
                <h1>DATA UNIT</h1>
                <p>Pengelolaan Data Master Unit Rumah Sakit</p>
            </div>

            <!-- BARIS PENCARIAN & TOMBOL UTAMA -->
            <div class="control-bar">
                <form action="" method="POST" class="search-box">
                    <input type="text" name="keyword" class="input-search" placeholder="Cari kode atau nama unit..." value="<?php echo htmlspecialchars($keyword); ?>">
                    <button type="submit" name="cari" class="btn-search">Cari</button>
                </form>
                
                <div class="action-right">
                    <button type="button" class="btn-add" onclick="bukaModalTambah()">+ Tambah Unit Baru</button>
                </div>
            </div>

            <!-- TABEL DATA MASTER UNIT -->
                            ...
                            while($row = mysqli_fetch_assoc($hasil)) {
                                ...
                                echo "<td>
                                        <div class='action-container'>
                                            <button type='button' class='btn-action btn-edit' onclick=\"bukaModalEdit('".$row['id']."', '".addslashes($row['kode_unit'])."', '".addslashes($row['nama_unit'])."', '".$row['status']."')\" title='Edit Data Unit'>✏️</button>
                                            <a href='master_unit.php?hapus=".$row['id']."' class='btn-action btn-delete' onclick=\"return confirm('Apakah Anda yakin ingin menghapus unit ".$row['nama_unit']."?')\" title='Hapus Unit'>🗑️</a>
                                        </div>
                                      </td>";
                                echo "</tr>";
                            }
                        } else {
                            echo "<tr><td colspan='5' style='padding: 30px; color: #94a3b8;'>Tidak ada data unit yang ditemukan.</td></tr>";
                        }
                        ?>
                    </tbody>
                </table>
    ...
    <div id="modalTambah" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.4); backdrop-filter: blur(4px); justify-content: center; align-items: center; z-index: 999;">
        <div style="background: white; padding: 30px; border-radius: 16px; width: 100%; max-width: 450px; display: flex; flex-direction: column; gap: 15px; box-shadow: 0 10px 25px rgba(0,0,0,0.1);">
            <h3 style="color:#1e293b; border-bottom:2px solid #f1f5f9; padding-bottom:10px; font-weight:700;">Tambah Unit Baru</h3>
            <form action="" method="POST" style="display: flex; flex-direction: column; gap: 12px;">
                <div style="display: flex; flex-direction: column; gap: 4px;">
                    <label style="font-size: 11px; font-weight: 700; color: #94a3b8;">KODE UNIT</label>
                    <input type="text" name="kode_baru" placeholder="Contoh: UNT-01" class="table-input" style="width: 100%; padding: 10px; border: 1px solid #ced4da; border-radius: 6px;" required>
                </div>
                <div style="display: flex; flex-direction: column; gap: 4px;">
                    <label style="font-size: 11px; font-weight: 700; color: #94a3b8;">NAMA UNIT RUMAH SAKIT</label>
                    <input type="text" name="nama_baru" placeholder="Contoh: Unit Gawat Darurat (UGD)" class="table-input" style="width: 100%; padding: 10px; border: 1px solid #ced4da; border-radius: 6px;" required>
                </div>
                <div style="display: flex; gap: 10px; justify-content: flex-end; margin-top: 10px;">
                    ...
                    <button type="submit" name="simpan_unit_baru" style="background:#2ecc71; color:white; padding:8px 16px; border:none; border-radius:6px; cursor:pointer; font-weight:700;">Simpan</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="modalEdit" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.4); backdrop-filter: blur(4px); justify-content: center; align-items: center; z-index: 999;">
        <div style="background: white; padding: 30px; border-radius: 16px; width: 100%; max-width: 450px; box-shadow: 0 10px 25px rgba(0,0,0,0.1); display: flex; flex-direction: column; gap: 15px;">
            <h3 style="color:#1e293b; border-bottom:2px solid #f1f5f9; padding-bottom:10px; font-weight:700;">Ubah Data Master Unit</h3>
            <form action="" method="POST" style="display: flex; flex-direction: column; gap: 12px;">
                <input type="hidden" id="edit_id" name="id_unit">
                
                <div style="display: flex; flex-direction: column; gap: 4px;">
                    <label style="font-size: 11px; font-weight: 700; color: #94a3b8;">KODE UNIT</label>
                    <input type="text" id="edit_kode" name="kode_unit" placeholder="Kode Unit" class="table-input" style="width: 100%; padding: 10px; border: 1px solid #ced4da; border-radius: 6px;" required>
                </div>
                
                <div style="display: flex; flex-direction: column; gap: 4px;">
                    <label style="font-size: 11px; font-weight: 700; color: #94a3b8;">NAMA UNIT RUMAH SAKIT</label>
                    <input type="text" id="edit_nama" name="nama_unit" placeholder="Nama Unit" class="table-input" style="width: 100%; padding: 10px; border: 1px solid #ced4da; border-radius: 6px;" required>
                </div>
                
                <div style="display: flex; flex-direction: column; gap: 4px;">
                    <label style="font-size: 11px; font-weight: 700; color: #94a3b8;">STATUS AKTIVITAS</label>
                    <select id="edit_status" name="status_unit" class="table-input" style="width: 100%; padding: 10px; border: 1px solid #ced4da; border-radius: 6px;">
                ...
                <div style="display: flex; gap: 10px; justify-content: flex-end; margin-top: 10px;">
                    ...
                    <button type="submit" name="update_unit" style="background:#3182ce; color:white; padding:8px 16px; border:none; border-radius:6px; cursor:pointer; font-weight:700;">Simpan</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        function bukaModalEdit(id, kode, nama, status) {
